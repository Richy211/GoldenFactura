		...
		if($_POST['action'] == 'searchCliente')
		{
			...
			if(!empty($_POST['cliente'])){
				...
				$query = mysqli_query($conection,"SELECT * FROM cliente 
					WHERE nit LIKE '$nit' and estatus = 1");
				mysqli_close($conection);
				$result = mysqli_num_rows($query);
	...
	if($_POST['action'] == 'addProductoDetalle'){
		...
		if(empty($_POST['producto']) || empty($_POST['cantidad']))
		{
			...
			if($result > 0){
				...
				//este while devuelve todo lo que trae el procedimiento almacenado
					...
					$detalleTabla .= '<tr>
									...
									<td class="">
										<a class="link_delete" href="#" onclick="event.preventDefault();
										 del_product_detalle('.$data['correlativo'].');"> <i class="fa fa-trash"></i></a>
									</td>
				</tr>';
...
//Extrae datos del detalle_temp
	...
	if($_POST['action'] == 'anularVenta'){
		...
		$query_del = mysqli_query($conection,"DELETE FROM detalle_temp 
			WHERE token_user = '$token' ");
		mysqli_close($conection);
		if($query_del){
			echo 'ok';
